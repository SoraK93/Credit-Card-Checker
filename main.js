...
function validateCred(card) {
  const checkCard = [...card].reverse();
  let sumDigit = checkCard[0];

  for (let i = 1; i < checkCard.length; i++) {
    if (i % 2 === 0) {
      sumDigit += checkCard[i];
      continue;
    }

    let currDigit = checkCard[i] * 2;
    currDigit > 9 ? (sumDigit += currDigit - 9) : (sumDigit += currDigit);
  }

  return sumDigit % 10 === 0 ? true : false;
}

// get all invalid cards in an array
...
function idInvalidCardCompanies(cards) {
  const invalidCompanyCard = new Set();
  const companyList = {
  ...
  for (let card of cards) {
    let currCardCompany = companyList[card[0]];
    if (currCardCompany) {
      invalidCompanyCard.add(currCardCompany);
    } else {
